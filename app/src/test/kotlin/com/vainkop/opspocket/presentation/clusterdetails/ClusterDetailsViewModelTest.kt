...
class ClusterDetailsViewModelTest {
    ...
    private fun createViewModel(
        ...
        val savedStateHandle = SavedStateHandle(mapOf("clusterId" to clusterId))
        ...
        return ClusterDetailsViewModel(savedStateHandle, getDetailsUseCase, triggerRebalancingUseCase)
    }
    ...
    fun `state is Error with network message on NetworkError`() = runTest {
        val repository = createRepository(
            getClusterDetailsResult = AppResult.NetworkError(IOException("Timeout")),
        )
        val viewModel = createViewModel(repository)
    ...
    fun `rebalancing network error updates state to Failure`() = runTest {
        val repository = createRepository(
            getClusterDetailsResult = AppResult.Success(readyCluster),
            createPlanResult = AppResult.Success(samplePlan),
            executePlanResult = AppResult.NetworkError(IOException("Network down")),
        )
        val viewModel = createViewModel(repository)
